...
class Utils{
    ...
    public static function randomiseDungeonQuest($player, $dungeon, $dungeon_id, $stage, $mode){
        $npcidentifier = NPCList::$DUNGEON[$dungeon_id][$stage];
        $lvl = $player->getLVL();
        
        $coins = ($lvl + 5)*($lvl/2);
        $coins *= Config::get("constants.fight_quest_reward_coin_scale_hard");
        $xp = (10 * $lvl * 5) + (random(0.5, 1) * 550);
        $xp *= Config::get("constants.fight_quest_reward_xp_scale_hard");
        //Guild booster
        ...
        $identifier = "dungeon{$dungeon_id}_quest".($stage+1);
        return [
    ...
    public static function refreshShopItems($player){
		...
		for($i=0; $i < 9; $i++){
			...
			if($item['type'] == Item::$TYPE_ID['sidekick']){
				$quality = rand(1,3);
				$shpit->quality = $quality;
				$shpit->stat_critical_rating *= $quality;
				$shpit->stat_dodge_rating *= $quality;
				$shpit->stat_stamina *= $quality;
				$shpit->stat_strength *= $quality;
				$shpit->stat_weapon_damage *= $quality;
			}			
	...
	public static function duelLeagueFightWinReward($honor1, $honor2, $guildDuelBooster=1, $guildDuelArtifactBooster=1){
				
		$loc_4 = $guildDuelBooster;
		$loc_8 = $guildDuelArtifactBooster;
		$loc_5 = 1 / (1 + pow(10, ($honor2 - $honor1) / 400));
		$loc_7 = Config::get('constants.league_elo_calculation_k');
		$loc_6 = $loc_7 * (1 - $loc_5);
		$loc_6 = $loc_6 * $loc_4 * $loc_8;
    ...
    public static function duelLeaguePointsLostReward($honor1, $honor2){
		 $loc_4 = 1 / (1 + pow(10, ($honor2 - $honor1) / 400));
		 $loc_3 = Config::get('constants.league_elo_calculation_k');
		 return (round(- $loc_3 * (1 - $loc_4)));
    }
	...
	// Item pattern
    public static function addItemToPattern($player, $item) {
        ...
        $itemPatternFiltered = array_filter(
            ItemsList::$ITEMS[$itemType], 
            function($itemList) use ($item) { 
                return $itemList['identifier'] == $item->identifier; 
            }
        );
        
        if (empty($itemPatternFiltered)) {
            return;
        }
        
        $itemData = reset($itemPatternFiltered);
        if (empty($itemData['item_pattern'])) {
            return;
        }
        
        $patternName = $itemData['item_pattern'];
        $itemPattern = GameSettings::getConstant("item_pattern.{$patternName}");
        
        if (empty($itemPattern)) {
            return;
        }
        
        $currentPatternValues = $player->current_item_pattern_values;
        if (!isset($currentPatternValues[$patternName])) {
            $currentPatternValues[$patternName] = [
                'collected_items' => [],
                'value' => 0
            ];
        }
        
        if (!in_array($item->identifier, $currentPatternValues[$patternName]['collected_items'])) {
            $currentPatternValues[$patternName]['collected_items'][] = $item->identifier;
            $currentPatternValues[$patternName]['value'] += 1;
            $player->current_item_pattern_values = $currentPatternValues;

            \Srv\DB::sql('UPDATE `character` SET `current_item_pattern_values` = ? WHERE `id` = ?', [
                json_encode($currentPatternValues),
                $player->character->id
            ]);

            Core::req()->append['current_item_pattern_values'] = $player->current_item_pattern_values;
        }
    }

    // Item owned templates
    public static function getOwnedItemTemplates($player, $type) {
        if ($type < 1 || $type > 7) {
            return [];
        }

        $result = \Srv\DB::sql('SELECT `owned_item_templates` FROM `character` WHERE `id` = ?', [$player->character->id])->fetch();
        $templates = json_decode($result['owned_item_templates'] ?? '{}', true) ?? [];
        return $templates[$type] ?? [];
    }

    public static function addItemToOwnedTemplates($player, $item) {
        $sewingMachineReqLevel = GameSettings::getConstant('sewing_machine_req_level');
        ...
        if (!in_array($item->identifier, $templates[$item->type])) {
            $templates[$item->type][] = $item->identifier;
            
            \Srv\DB::sql('UPDATE `character` SET `owned_item_templates` = ? WHERE `id` = ?', [
